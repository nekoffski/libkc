...
TEST_F(
    UtilsTests, givenContainerWithDesiredElement_whenQueringForIndexOf_shouldReturnCorrectIndex
) {
    EXPECT_EQ(kc::core::indexOf(elements, 4), 3);
}

TEST_F(
    UtilsTests, givenContainerWithoutDesiredElement_whenQueringForIndexOf_shouldReturnDefaultValue
) {
...
TEST_F(
    UtilsTests, givenConstCharPointerVectorWithValue_whenCheckingIfContainsValue_shouldReturnTrue
) {
    std::string text = "bbb";
