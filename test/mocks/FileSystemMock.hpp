...
struct FileSystemMock : public FileSystem {
    ...
    MOCK_METHOD(void, writeFile, (const Path& path, const std::string& buffer, FileSystem::WritePolicy), (const, override));
    MOCK_METHOD(std::string, readFile, (const Path& path), (const, override));
    MOCK_METHOD(std::filesystem::file_time_type, getLastFileModificationTime, (const Path& path), (const, override));
};
