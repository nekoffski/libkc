...
EventEmitter::EventEmitter(EventContainer& eventContainer) : m_eventContainer(eventContainer) {}
...
void EventEmitter::pushEvent(std::shared_ptr<Event> event, CategoryToEventQueue& categoryMap) {
    ...
    if (not categoryMap.contains(categoryTypeIndex)) categoryMap[categoryTypeIndex] = EventQueue{};
