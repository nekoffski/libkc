    ...
    public:
        ...
        public:
            Iterator(std::size_t index, ValueIterator&& valueIterator)
                : m_index(index)
                , m_valueIterator(std::forward<ValueIterator>(valueIterator)) {
            }

            Value operator*() const {
                return Value(m_index, *m_valueIterator);
            }
        ...
        explicit EnumerateRange(T& container)
            : m_begin(0, container.begin())
            , m_end(container.size(), container.end()) {
        }
