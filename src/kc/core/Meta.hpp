...
struct is_one_of3 : std::integral_constant<
                        bool, std::is_same<T, U>::value || std::is_same<T, V>::value ||
...
template <bool Condition, typename U> using enable_if = typename std::enable_if<Condition, U>::type;
...
template <typename T> using get_iterator_type = typename std::decay_t<T>::iterator;
