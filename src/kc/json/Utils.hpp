...
enable_for(glm::vec4) tryToExtractValue(const Node& node, const std::string& messagePrefix) {
    ...
    return glm::vec4{node[0].asFloat(), node[1].asFloat(), node[2].asFloat(), node[3].asFloat()};
}
...
struct ValueWrapper<Error, T, typename std::enable_if_t<std::is_arithmetic_v<T>>>
    ...
    ValueWrapper&& inRange(T minValue, T maxValue) && { MOVE_THIS.min(minValue).max(maxValue); }
};
   ...
   public:
    ...
    template <typename T> ValueWrapper<Error, T> ofType() && {
        ...
        return ValueWrapper<Error, T>{
            tryToExtractValue<Error, T>(m_node.get(m_name.value(), {}), kc_assertMessagePrefix),
            m_name.value()};
    }
