...
static void core_Range_native(benchmark::State& state) {
    ...
    for (auto _ : state)
        for (int i = 0; i < innerIterations; ++i) benchmark::DoNotOptimize(i);
}
...
static void core_Range_range(benchmark::State& state) {
    auto innerIterations = (int)state.range(0);
    for (auto _ : state)
        for (auto i : kc::core::range(innerIterations)) benchmark::DoNotOptimize(i);
}
...
static void core_Range_stepIntegerNative(benchmark::State& state) {
    auto innerIterations = (int)state.range(0);
    for (auto _ : state)
        for (int i = 0; i < innerIterations; i += 3) benchmark::DoNotOptimize(i);
}
